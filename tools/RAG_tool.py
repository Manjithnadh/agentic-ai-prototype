...
def query_documents(user_query: str, folder_path: str = "uploaded_files"):
    rag_session.initialize(folder_path)
    return rag_session.query(question)


def rag_tool(query: str, conversation: str = ""):
    """RAG tool interface for the agent"""
    try:
        if conversation:
            enhanced_query = f"Conversation context: {conversation}\nQuestion: {query}"
        else:
            enhanced_query = query

        return query_documents(enhanced_query)
